...
const { digest } = require('../backend/src/general/releases/release-contract.ts')
const { createCoursePreview, publishReviewedRelease, publishedCourse, authorizedPreview, recordReleaseCheck } = require('../backend/src/general/releases/release-service.ts')
...
try {
  ...
  await assert.rejects(recordReleaseCheck(preview.course.release.courseReleaseId,'web',{adminId:Number(admin.id)}, {lineId:'stable-1',startUs:1000000,endUs:2000000,startedPositionUs:1000000,finishedPositionUs:1999999,playbackRate:1,adapter:'browser-native',playbackContractVersion:1}),/误差/)
  ...
  await assert.rejects(sequelize.transaction(t=>publishReviewedRelease(exerciseId,Number(admin.id),lines,t)),/播放一致性确认/)
  for(const platform of ['web','ios','android'])await recordReleaseCheck(preview.course.release.courseReleaseId,platform,{adminId:Number(admin.id)},{lineId:'stable-1',startUs:1000000,endUs:2000000,startedPositionUs:1000000,finishedPositionUs:2000000,playbackRate:1,adapter:platform==='web'?'browser-native':'native-timed-v1',playbackContractVersion:1})
  ...
  assert.equal(published.release.courseReleaseId,preview.course.release.courseReleaseId)
  ...
  console.log('PASS: immutable published snapshot, atomic pointer, three-platform gate, bound media, preview authorization/expiry, stable ids')
