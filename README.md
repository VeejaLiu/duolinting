...
- 通过「任务广场」自助领取字幕校对任务（也可由管理员指派）：领取后锁定、保存草稿自动续期 48 小时，超时未保存自动释放回任务池；完成校对与二审后，贡献者在课程中署名。
...
生产环境的学习账号注册使用邮件验证码确认邮箱；忘记密码时通过同一邮件服务发送一次性重置码，并在重置成功后撤销全部旧会话。验证码仅以带服务器密钥的哈希形式保存，10 分钟过期；同一邮箱与来源 IP 发送后冷却 60 秒，每小时最多发送 6 次。未配置 Resend 的非生产自托管环境仍可直接注册，但无法使用找回密码。
