...
test("server-renders the DuolinTing official site", async () => {
  ...
  assert.match(html, /hero-actions"><a class="button button-primary" href="https:\/\/app\.duolinting\.cn\/api\/v1\/media\/android-apk"/);
  assert.match(html, /header-cta header-download-cta/);
...
test("publishes the signed Android release on the download page", async () => {
  ...
  assert.equal(response.status, 200);
  const html = await response.text();
  assert.match(html, /DuolinTing Android 版，立即下载/);
  assert.match(html, /网页学习端/);
  assert.match(html, /也可以使用网页版/);
  assert.match(html, /iOS 等待 App Store 上架/);
  ...
  assert.match(html, /button button-primary chrome-primary-action/);
  assert.match(html, /chrome-secondary-action/);
