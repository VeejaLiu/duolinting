...
import {
    expireOverdueSelfClaims,
    notifyExpiringSelfClaims,
} from '../general/admin/collaboration-service';

const logger = new Logger(__filename);

/**
 * 低频清扫器：释放过期的自助领取任务，并在到期前提醒。
...
export function startWorkflowClaimSweeper(intervalMs = 10 * 60 * 1000) {
    const runOnce = async () => {
        try {
            const now = new Date();
            await notifyExpiringSelfClaims(now);
            const expired = await expireOverdueSelfClaims();
            if (expired.length > 0) {
                logger.info(`[workflow-claim] 已释放 ${expired.length} 门超时未提交的自助领取课程`);
            }
        } catch (error) {
