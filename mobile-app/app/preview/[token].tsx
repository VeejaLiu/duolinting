...
export default function ReleasePreviewScreen() {
  ...
  const play = async () => {
    if (!line) return
    const id = ++attempt.current; setError('')
    const session = playback.playRangeSession(line, repeats)
    const result = await session.finished
    if (attempt.current !== id) return
    if (result.reason === 'failed' || result.reason === 'timeout') setError(t(playback.nativePlaybackAvailable ? 'study.playbackFailed' : 'preview.updateRequired'))
  }
  ...
  return <SafeScreen><AppScrollView contentContainerStyle={{ padding: 16 }}>
    ...
    {course && line && <>
      ...
      {button(`${t('preview.repeat')} ${repeats}`, () => { attempt.current++; playback.pause(); setRepeats(repeats === 3 ? 1 : repeats + 1) })}
      {button(t('preview.rate')+` ${rate}×`, () => { attempt.current++; playback.pause(); setRate(rate === 2 ? 0.5 : rate + 0.5) })}
      {button(t('preview.previous'), () => { attempt.current++; playback.pause(); setIndex(index-1) }, index===0)}
      {button(t('preview.next'), () => { attempt.current++; playback.pause(); setIndex(index+1) }, index===course.lines.length-1)}
    </>}
